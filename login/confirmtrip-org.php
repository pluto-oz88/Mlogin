...
<body style="font-family: 'Montserrat', sans-serif; font-weight: 700; font-size: 1em;">
    ...
    <div style=" width: 100%;max-width: 390px;min-width: 320px;margin: 0;padding: 10px;color: #314455; background-color: white;box-shadow: 12px 12px 20px rgba(0, 0, 0, 0.5);border-radius: 4px; overflow: hidden;">
        ...
        if ($result = $conn->query($sql)) {
            if ($result->num_rows > 0) {
                echo "Query was successful and returned results";
                // Output the record data
                while ($row = mysqli_fetch_assoc($result)) {



                    echo "<p>PostJourney: " . $row["journeyNo"] . "</p>";

                    $newDate = date("d-m-Y", strtotime($row['pickupDate']));
                    $rqDate = date("d-m-Y g:i A", strtotime($row['requestDate']));
                    $urlP = $row['pickupURL'];
                    $urlD = $row['dropoffURL'];
                    ...
                    $date->modify('-2 days');
                    // Format the new date in the same format
                    $payDate = $date->format('d-m-Y');

                    echo "<p>Journey No.: " . $row["journeyNo"] . "</p>";
                    ...
                    $duration = $row['duration']; // Assuming the duration is in the format "83 minutes"
                    ...
                    $start_time = new DateTime($row['pickupDate'] . "T" . $row['pickupTime'] . ".000+10:00");
                    $end_time = clone $start_time;
                    $end_time->add(new DateInterval('PT' . $duration_minutes . 'M'));

                    $formatted_end_time = $end_time->format('Y-m-d\TH:i:s.000P');

                    $_SESSION['event_name'] = "GDrive Booking - Journey No. "  . $row['journeyNo'];
                    $_SESSION['start_time'] = $row['pickupDate'] . " " . $row['pickupTime'];
                    $_SESSION['duration'] = $duration_minutes;
                    $_SESSION['location'] = $row['pickupAddress'];
                    $_SESSION['description'] = $row['rideNotes'];
                    $_SESSION['journeyNo'] = $row['journeyNo'];
                    $journeyNo = $row['journeyNo'];
                }
            } else {
                echo 'No result';
                exit();
    ...
    <div class="container">
        ...
        <div class="blurb">
            <p>
                Your trip has been submitted and you have been sent a confirmation email. To save your ride to your calendar click on the link below.<br>Thank you for using GDrive.
            </p>
        </div>
